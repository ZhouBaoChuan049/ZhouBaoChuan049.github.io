...
export function setWallpaperMode(mode: WALLPAPER_MODE): void {
    ...
    // Notify listeners (e.g. wallpaperSwitch component) of the mode change
    if (typeof document !== 'undefined') {
        document.dispatchEvent(new CustomEvent('wallpaper-mode-changed', { detail: { mode } }));
    }
}

// ---- Auto wallpaper switch based on scroll position ----

let autoSwitchEnabled = true;
let lastAutoSwitchTime = 0;
const AUTO_SWITCH_COOLDOWN = 800;

function getAutoSwitchThreshold(): number {
    if (typeof window === 'undefined') return 0;
    return window.innerHeight * (BANNER_HEIGHT / 100);
}

/** Call this when the user manually selects a wallpaper mode. Disables auto-switch when "none" is picked. */
export function markManualSwitch(mode: WALLPAPER_MODE): void {
    autoSwitchEnabled = mode !== WALLPAPER_NONE;
}

function autoSwitchBasedOnScroll(): void {
    if (!autoSwitchEnabled) return;

    const currentMode = document.documentElement.getAttribute('data-wallpaper-mode') as WALLPAPER_MODE;
    if (currentMode !== WALLPAPER_BANNER && currentMode !== WALLPAPER_FULLSCREEN) return;

    const now = Date.now();
    if (now - lastAutoSwitchTime < AUTO_SWITCH_COOLDOWN) return;

    const scrollTop = document.documentElement.scrollTop || window.scrollY;
    const threshold = getAutoSwitchThreshold();

    if (currentMode === WALLPAPER_BANNER && scrollTop > threshold) {
        lastAutoSwitchTime = now;
        setWallpaperMode(WALLPAPER_FULLSCREEN);
    } else if (currentMode === WALLPAPER_FULLSCREEN && scrollTop < threshold * 0.3) {
        lastAutoSwitchTime = now;
        setWallpaperMode(WALLPAPER_BANNER);
    }
}

/** Initialize scroll-based auto switching between banner and fullscreen wallpaper modes. */
export function initAutoWallpaperSwitch(): void {
    if (typeof window === 'undefined') return;

    // Respect stored "none" preference
    const storedMode = getStoredWallpaperMode();
    if (storedMode === WALLPAPER_NONE) {
        autoSwitchEnabled = false;
    }

    // Fix initial state if scroll position doesn't match stored mode
    const scrollTop = document.documentElement.scrollTop || window.scrollY;
    const threshold = getAutoSwitchThreshold();
    if (storedMode === WALLPAPER_BANNER && scrollTop > threshold) {
        setWallpaperMode(WALLPAPER_FULLSCREEN);
    } else if (storedMode === WALLPAPER_FULLSCREEN && scrollTop < threshold * 0.3) {
        setWallpaperMode(WALLPAPER_BANNER);
    }
